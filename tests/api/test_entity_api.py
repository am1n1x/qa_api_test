...
class TestEntityApi:
    ...
    def test_create_entity(self, api_client: EntityApiClient):
        ...
        with allure.step("Подготовка (Arrange): Генерируем уникальные тестовые данные для создания сущности"):
    ...
    def test_get_entity(self, api_client: EntityApiClient, created_entity_id: tuple):
        """Тест на получение сущности: GET /api/get/{id}."""
        entity_id, payload_data = created_entity_id
        ...
        with allure.step("Действие (Act): Отправляем запрос на получение сущности"):
            response = api_client.get_entity(entity_id)

        with allure.step("Проверка (Assert): Проверяем статус-код и сверяем данные с исходными"):
            assert response.status_code == 200
            ...
            assert entity_data.title == payload_data["title"]
            assert entity_data.addition.additional_info == payload_data["addition"]["additional_info"]


    @allure.story("Получение списков")
    @allure.title("Получение списка всех сущностей")
    def test_get_all_entities(self, api_client: EntityApiClient, created_entity_id: tuple):
        """Тест на получение списка сущностей: GET /api/getAll."""
        entity_id, _ = created_entity_id

        with allure.step("Подготовка (Arrange): Создана как минимум одна уникальная сущность через фикстуру"):
            pass
        ...
        with allure.step("Проверка (Assert): Проверяем статус-код и наличие нашей сущности в списке"):
            ...
            assert any(entity.id == entity_id for entity in list_of_entities)


    @allure.story("CRUD-операции")
    @allure.title("Обновление существующей сущности")
    def test_update_entity(self, api_client: EntityApiClient, created_entity_id: tuple):
        """Тест на обновление сущности: PATCH /api/patch/{id}."""
        entity_id, _ = created_entity_id

        with allure.step("Подготовка (Arrange): Генерируем новые уникальные данные для обновления"):
        ...
        with allure.step("Действие (Act): Отправляем запрос на обновление сущности"):
            response = api_client.update_entity(entity_id, updated_payload)
    ...
    def test_delete_entity(self, api_client: EntityApiClient):
        ...
        with allure.step("Подготовка (Arrange): Создаем уникальную сущность специально для удаления"):
            payload = create_entity_payload()
